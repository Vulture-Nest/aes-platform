...
void main() {
  ApprovalsCubit build(FakeApprovalsRepository repo,
  ...
  test('load populates the inbox', () async {
    final cubit = build(
        FakeApprovalsRepository(items: [moneyApproval(), nonMoneyApproval()]));
    await cubit.load();
  ...
  test(
      'approving a money item requires biometric confirmation and calls the API',
      () async {
    ...
    final result =
        await cubit.decide(cubit.state.items.first, ApprovalDecision.approved);
  ...
  test('a declined biometric cancels a money approval without calling the API',
      () async {
    final repo = FakeApprovalsRepository(items: [moneyApproval()]);
  ...
  test('non-money items skip biometrics; reject/return pass a comment',
      () async {
  ...
  test('a failed decide surfaces the error and keeps the item', () async {
    final repo =
        FakeApprovalsRepository(items: [nonMoneyApproval()], failDecide: true);
    final cubit = build(repo);
    await cubit.load();
