...
void main() {
  Future<CommandCentreCubit> pump(WidgetTester tester,
      {required FakeAlertsRepository alerts}) async {
    ...
    final cubit = CommandCentreCubit(
        repository: FakeCommandCentreRepository(), alerts: alerts);
    addTearDown(cubit.close);
    await tester.pumpWidget(
      MaterialApp(
        theme: AppTheme.light(),
        home: BlocProvider.value(
            value: cubit, child: const CommandCentreScreen()),
      ),
    );
  ...
  testWidgets('shows the verdict banner, a metric and the alert feed',
      (tester) async {
    await pump(tester,
        alerts: FakeAlertsRepository(
            alerts: [dangerAlert('Cash runway negative')]));
  ...
  testWidgets('acknowledging an alert removes it from the feed',
      (tester) async {
    final alerts =
