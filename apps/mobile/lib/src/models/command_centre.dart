...
class CommandCentre {
  ...
  factory CommandCentre.fromJson(Map<String, dynamic> json) =>
      CommandCentre(json);
  ...
  double? get cashUsd =>
      _d(_get(['cashPosition', 'usdEquivalent', 'official', 'totalUsd']));
  ...
  double? get obligationsUsd =>
      _d(_get(['pendingObligations', 'usdEquivalent', 'obligations']));
  double? get unfundedGapUsd =>
      _d(_get(['pendingObligations', 'usdEquivalent', 'unfundedGap']));
  ...
  double? get taxWithInterest =>
      _d(_get(['taxExposure', 'assessmentTotals', 'totalWithInterest']));

  /// Outstanding receivables (from the health-verdict drivers).
  double? get receivables =>
      _d(_get(['healthVerdict', 'drivers', 'receivables']));
}
