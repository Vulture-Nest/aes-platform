...
class _DirectorScreenState extends State<DirectorScreen> {
  ...
  Future<void> _raise() async {
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) =>
            BlocProvider.value(value: cubit, child: const _WithdrawalForm()),
      ),
    );
  }
  ...
  Future<void> _openDetail(DirectorWithdrawal w) async {
    ...
    final done = await showModalBottomSheet<String>(
      ...
      builder: (_) =>
          BlocProvider.value(value: cubit, child: _DetailSheet(withdrawal: w)),
    );
    if (done != null) {
      messenger
        ..hideCurrentSnackBar()
      ...
      body: BlocBuilder<DirectorCubit, DirectorState>(
        builder: (context, state) {
          ...
          if (state.items.isEmpty) {
            return EmptyState(
              icon:
                  state.error != null ? Icons.cloud_off : Icons.gavel_outlined,
                  ...
                  Card(
                    child: ListTile(
                      ...
                      leading: IconBadge(
                        ...
                        background:
                            _statusColor(w.status).withValues(alpha: 0.14),
                      ),
                      title: Text(
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  enabled: !_busy,
                  decoration:
                      const InputDecoration(labelText: 'Destination account'),
...
class _DetailSheetState extends State<_DetailSheet> {
  ...
  Future<void> _submit() async {
    setState(() => _busy = true);
    final error =
        await context.read<DirectorCubit>().submit(widget.withdrawal.id);
  ...
  Future<void> _complete() async {
    if (_reference.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Enter the transfer reference')));
      return;
    }
    setState(() => _busy = true);
        ...
        children: [
          ...
          if (w.awaitingTransfer) ...[
            ...
            DropdownButtonFormField<String>(
              ...
              onChanged:
                  _busy ? null : (v) => setState(() => _method = v ?? 'EFT'),
            ),
            const SizedBox(height: 12),
            TextField(
              ...
              enabled: !_busy,
              decoration:
                  const InputDecoration(labelText: 'Transfer reference'),
            ),
            const SizedBox(height: 16),
            FilledButton.icon(
