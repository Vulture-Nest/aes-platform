...
export function PipelineTab() {
  const { data: board, isLoading, refetch } = useGetCrmOpportunityBoardQuery();
  const { data: orgs } = useGetCrmOrganisationsQuery();
  const { data: users } = useGetCrmUsersQuery();
  const { data: clients } = useGetCrmClientsQuery();
  const [create, createState] = useCreateCrmOpportunityMutation();
  const [move] = useMoveCrmOpportunityMutation();
  const [convert, convertState] = useConvertCrmOpportunityMutation();
