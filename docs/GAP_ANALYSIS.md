...
> - ✅ **Tier-4 sweep (G16–G25) done** — order health/profit on the order API (G16); partial-order-servicing milestones → PARTIALLY_SERVICED (G18); income-tax provision info line (G19); Settings-sheet import + granular per-line $0.01 parity (G23); mandatory fiscal-invoice guard, default-off (G20); timesheet reopen-unlock + auto-generated compliance obligations (G22); petty-cash conversions report + travel overspend (G25); structured SHE module + admin page (G21); notification-config UI, admin + web prefs (G24). *(Skipped, still open: in-app help pages, hard-coded USD/ZWG refactor.)*
> - Full suite: **82 suites / 805 tests green**; admin+web build green; parity still ACT (now with $0.01 granular checks). Remaining: G13 (Fabric, external), G15, G26–G28.
